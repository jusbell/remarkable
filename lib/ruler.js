...
Ruler.prototype.at = function (name, fn, options) {
  ...
  this.rules[index].anchor = opt.anchor ? (Array.isArray(opt.anchor) ? opt.anchor : [ opt.anchor ]) : [];
  this.cache = null;
};
...
Ruler.prototype.before = function (beforeName, ruleName, fn, options) {
  ...
  this.rules.splice(index, 0, {
    ...
    fn: fn,
    alt: opt.alt || [],
    anchor: opt.anchor ? (Array.isArray(opt.anchor) ? opt.anchor : [ opt.anchor ]) : []
  });
...
Ruler.prototype.compile = function () {
  ...
  chains.forEach(function (chain) {
    self.cache[chain] = {
      0:        [], // all rules (any anchor)
      0xFFFFFF: []  // fallback for rules with unmatched anchor
    };
    self.rules.forEach(function (rule) {
      if (!rule.enabled) { return; }
      ...
      self.cache[chain][0].push(rule.fn);

      if (rule.anchor.length === 0) {
        self.cache[chain][0xFFFFFF].push(rule.fn);
      }

      rule.anchor.forEach(function (a) {
        if (a === 0xFFFFFF) { return; }
        if (!self.cache[chain][a]) { self.cache[chain][a] = []; }
        self.cache[chain][a].push(rule.fn);
      });
    });
  });
};
...
Ruler.prototype.getRules = function (chainName) {
  ...
  return this.cache[chainName][0];
};


// Get rules list as array of functions, filtering by anchor.
//
Ruler.prototype.getAnchoredRules = function (chainName, anchor) {
  if (this.cache === null) {
    this.compile();
  }

  if (typeof this.cache[chainName][anchor] === 'undefined') {
    return this.cache[chainName][0xFFFFFF]; // Default fallback with all anchored rules excluedd
  }

  return this.cache[chainName][anchor];
};
